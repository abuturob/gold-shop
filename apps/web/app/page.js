...
const slides = [
  { label: 'Uzuk', sub: '585 proba · 3.2g', price: '2 800 000' },
  { label: 'Marjon', sub: '750 proba · 12g', price: '9 500 000' },
  { label: 'Braslet', sub: '585 proba · 8g', price: '6 200 000' },
  { label: 'Sirga', sub: '585 proba · 2.1g', price: '1 900 000' },
]
    ...
    <div style={{position: 'relative', width: '100%', height: '100%', background: '#0f0d08'}}>
      {slides.map((slide, i) => (
        <div key={i} style={{
          ...
          opacity: current === i ? 1 : 0, transform: current === i ? 'scale(1)' : 'scale(1.05)',
          transition: 'all 0.8s ease',
          background: 'radial-gradient(ellipse at center, #2a2000 0%, #1a1500 40%, #0a0800 100%)',
          gap: '16px'
        }}>
          <div style={{fontSize: '7rem', filter: 'drop-shadow(0 0 40px rgba(201,168,76,0.3))', marginBottom: '8px'}}>{icons[i]}</div>
          <div style={{fontFamily: 'Cormorant Garamond, serif', color: '#F5F0E8', fontSize: '1.8rem', fontWeight: '300', letterSpacing: '2px'}}>{slide.label}</div>
          <div style={{fontFamily: 'Montserrat, sans-serif', color: '#555', fontSize: '0.7rem', letterSpacing: '2px', textTransform: 'uppercase'}}>{slide.sub}</div>
          <div style={{fontFamily: 'Cormorant Garamond, serif', color: '#C9A84C', fontSize: '1.4rem', fontWeight: '400', marginTop: '8px'}}>{slide.price} so'm</div>
        </div>
      ))}
      <div style={{position: 'absolute', bottom: '24px', left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: '8px'}}>
        {slides.map((_, i) => (
          <div key={i} onClick={() => setCurrent(i)} style={{width: i === current ? '24px' : '6px', height: '1px', background: i === current ? '#C9A84C' : '#333', transition: 'all 0.3s', cursor: 'pointer'}} />
        ))}
      </div>
    </div>
  )
    ...
    <main style={{background: '#080808', minHeight: '100vh', fontFamily: "'Georgia', serif", overflowX: 'hidden'}}>
      <style>{`
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        .btn-primary { display: inline-block; background: linear-gradient(135deg, #A07830, #C9A84C, #E8C96A); color: #000; padding: 16px 48px; text-decoration: none; font-family: 'Montserrat', sans-serif; font-size: 0.7rem; font-weight: 700; letter-spacing: 3px; text-transform: uppercase; transition: all 0.3s; }
        .btn-primary:hover { opacity: 0.9; transform: translateY(-1px); }
        .btn-secondary { display: inline-block; border: 1px solid #333; color: #888; padding: 16px 48px; text-decoration: none; font-family: 'Montserrat', sans-serif; font-size: 0.7rem; font-weight: 500; letter-spacing: 3px; text-transform: uppercase; transition: all 0.3s; }
        .btn-secondary:hover { border-color: #C9A84C; color: #C9A84C; }
        .feature-card { background: #0f0f0f; border: 1px solid #1e1e1e; padding: 40px 32px; transition: all 0.4s; position: relative; overflow: hidden; }
        .feature-card::before { content: ''; position: absolute; top: 0; left: 0; width: 2px; height: 0; background: linear-gradient(180deg, #C9A84C, transparent); transition: height 0.4s; }
        .feature-card:hover { border-color: #2a2a2a; transform: translateY(-4px); }
        .feature-card:hover::before { height: 100%; }
        .marquee-track { display: flex; gap: 80px; animation: marquee 20s linear infinite; white-space: nowrap; }
      ...
      <section style={{minHeight: '100vh', display: 'flex', alignItems: 'center', padding: '120px 60px 80px', position: 'relative', overflow: 'hidden'}}>
        <div style={{position: 'absolute', top: '20%', right: '8%', width: '500px', height: '500px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(201,168,76,0.06) 0%, transparent 70%)', pointerEvents: 'none'}} />
        ...
        <div style={{maxWidth: '600px', position: 'relative', zIndex: 1}}>
          ...
          <h1 style={{fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(3.5rem, 6vw, 6rem)', fontWeight: '300', lineHeight: '1.05', color: '#F5F0E8', letterSpacing: '-1px', marginBottom: '32px'}}>
            Eng Ishonchli<br/>
            <em style={{fontStyle: 'italic', color: '#C9A84C'}}>Oltin Bozori</em>
          </h1>
          <p style={{fontFamily: 'Montserrat, sans-serif', color: '#666', fontSize: '0.95rem', lineHeight: '1.8', maxWidth: '480px', marginBottom: '48px', fontWeight: '300'}}>
          ...
          <div style={{display: 'flex', gap: '48px', paddingTop: '48px', borderTop: '1px solid #1a1a1a'}}>
            {[
        ...
        <div style={{position: 'absolute', right: '0', top: '0', width: '45%', height: '100%', overflow: 'hidden', border: 'none', maskImage: 'linear-gradient(to right, transparent 0%, black 15%, black 85%, transparent 100%)', WebkitMaskImage: 'linear-gradient(to right, transparent 0%, black 15%, black 85%, transparent 100%)'}}>
          <HeroSlider />
        </div>
      </section>
        ...
        <div className="marquee-track">
          {[...Array(2)].map((_, j) => (
            ['♦ OLTIN', '♦ KUMUSH', '♦ PLATINA', '♦ XAVFSIZ YETKAZIB BERISH', '♦ DAVLAT NAZORATI', '♦ SERTIFIKATLANGAN', '♦ 24/7 QOLLAB-QUVVATLASH'].map((t, i) => (
              <span key={`${j}-${i}`} style={{fontFamily: 'Montserrat, sans-serif', fontSize: '0.65rem', fontWeight: '700', letterSpacing: '3px', color: '#000', textTransform: 'uppercase'}}>{t}</span>
            ))
          ))}
        </div>
      </div>
      ...
      {/* CTA */}
      <section style={{padding: '120px 60px', background: 'linear-gradient(135deg, #0f0d08, #0a0a0a, #0f0d08)', borderTop: '1px solid #1a1a1a', borderBottom: '1px solid #1a1a1a', textAlign: 'center'}}>
        ...
        <p style={{fontFamily: 'Montserrat, sans-serif', color: '#555', fontSize: '0.85rem', marginBottom: '48px', letterSpacing: '1px'}}>O'zbekistondagi eng ishonchli oltin platformasiga qo'shiling</p>
        ...
        <div style={{maxWidth: '1200px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start'}}>
          <div>
            ...
            <p style={{fontFamily: 'Montserrat, sans-serif', color: '#333', fontSize: '0.75rem', lineHeight: '1.8'}}>
              O'zbekiston № 1 oltin marketplace<br/>© 2026 Gold Shop. Barcha huquqlar himoyalangan.
            </p>
          </div>
